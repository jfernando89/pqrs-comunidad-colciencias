 ...
 * @property Expediente $expediente0
 * @property Pqrs[] $pqrs
 */
class Dependencia extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nombre, expediente', 'required'),
			array('expediente', 'numerical', 'integerOnly'=>true),
			array('nombre', 'length', 'max'=>30),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nombre, expediente', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'expediente0' => array(self::BELONGS_TO, 'Expediente', 'expediente'),
			'pqrs' => array(self::HAS_MANY, 'Pqrs', 'dependencia'),
		);
	}
